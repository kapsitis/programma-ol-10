...
| 1.3 | 7.–9. stunda, Vjeta teorēmas piemērs $\left\{\begin{smallmatrix} x + y = 8 \\ xy = -20 \end{smallmatrix}\right.$ | $\left\{\begin{smallmatrix} m_{1} \cdot m_{2} = -20 \\ m_{1} + m_{2} = -8 \end{smallmatrix}\right.$; saknes $m_1 = -10$, $m_2 = 2$; Atbilde. $(-10;\,2)$ un $(2;\,-10)$ | $\left\{\begin{smallmatrix} m_{1} \cdot m_{2} = -20 \\ m_{1} + m_{2} = 8 \end{smallmatrix}\right.$; saknes $m_1 = 10$, $m_2 = -2$; Atbilde. $(10;\,-2)$ un $(-2;\,10)$ | Vienādojumam $m^2 - 8m - 20 = 0$ ir $p = -8$, tātad $m_1 + m_2 = -p = 8$ (saskan ar sistēmas nosacījumu $x + y = 8$). Dokumentā dotā atbilde neapmierina sistēmu: $-10 + 2 = -8 \neq 8$. |
...
| 2.2 | 1. stunda, piemērs par sistēmu bez atrisinājuma | „jo, saskaitot divus vienādus skaitļus, nav iespējams iegūt dažādas summas" | „jo, saskaitot vienus un tos pašus divus skaitļus, nav iespējams iegūt divas dažādas summas" | „Divi vienādi skaitļi" nozīmētu $x = y$, kas no sistēmas $\left\{\begin{smallmatrix} x + y = 10 \\ x + y = 21 \end{smallmatrix}\right.$ neizriet; jāsaskaita viena un tā pati summa $x + y$. |
...
**3.3 🟡 Substitūcijas metodes piemēra tabula (5., 6. stunda).** Sistēma $\left\{\begin{smallmatrix} 2m - n = 1 \\ 3m + 2n = 5 \end{smallmatrix}\right.$ tabulā parādīta divas reizes pēc kārtas bez izmaiņām, un tikai trešajā solī redzama sistēma pēc reizināšanas ar 2. Otrajā attēlojumā jāpievieno pārveidojuma norāde (piem., „$| \cdot 2$" pie (I) vienādojuma) vai dublējošais bloks jādzēš.

**3.4 🟡 Vienots darbību pieraksts pie vienādojumiem.** Dokumentā jaukti lietoti apzīmējumi „$|:(-7)$", „$| \cdot 2$" un „$/:( - 7)$", „$/ \cdot (-1)$". Ieteicams visā materiālā (un darba lapās) lietot vienu pierakstu, piemēram, ar vertikālo svītru.
